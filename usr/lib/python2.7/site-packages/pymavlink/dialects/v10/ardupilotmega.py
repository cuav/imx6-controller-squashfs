...
MAVLINK_MSG_ID_GPS_ACCURACY = 225
...
class MAVLink_fence_status_message(MAVLink_message):
        '''
        Status of geo-fencing. Sent in extended       status stream
        when fencing enabled
        '''
...
class MAVLink_gps_accuracy_message(MAVLink_message):
        '''
        Accuracy statistics for GPS lock
        '''
        id = MAVLINK_MSG_ID_GPS_ACCURACY
        name = 'GPS_ACCURACY'
        fieldnames = ['instance', 'h_acc', 's_acc', 'h_vel_filt', 'v_vel_filt', 'p_drift', 'ekf_check_mask']
        ordered_fieldnames = [ 'h_acc', 's_acc', 'h_vel_filt', 'v_vel_filt', 'p_drift', 'instance', 'ekf_check_mask' ]
        format = '<fffffBB'
        native_format = bytearray('<fffffBB', 'ascii')
        orders = [5, 0, 1, 2, 3, 4, 6]
        lengths = [1, 1, 1, 1, 1, 1, 1]
        array_lengths = [0, 0, 0, 0, 0, 0, 0]
        crc_extra = 76

        def __init__(self, instance, h_acc, s_acc, h_vel_filt, v_vel_filt, p_drift, ekf_check_mask):
                MAVLink_message.__init__(self, MAVLink_gps_accuracy_message.id, MAVLink_gps_accuracy_message.name)
                self._fieldnames = MAVLink_gps_accuracy_message.fieldnames
                self.instance = instance
                self.h_acc = h_acc
                self.s_acc = s_acc
                self.h_vel_filt = h_vel_filt
                self.v_vel_filt = v_vel_filt
                self.p_drift = p_drift
                self.ekf_check_mask = ekf_check_mask

        def pack(self, mav):
                return MAVLink_message.pack(self, mav, 76, struct.pack('<fffffBB', self.h_acc, self.s_acc, self.h_vel_filt, self.v_vel_filt, self.p_drift, self.instance, self.ekf_check_mask))
...
mavlink_map = {
        ...
        MAVLINK_MSG_ID_GPS_ACCURACY : MAVLink_gps_accuracy_message,
...
class MAVLink(object):
        ...
        def gps_accuracy_encode(self, instance, h_acc, s_acc, h_vel_filt, v_vel_filt, p_drift, ekf_check_mask):
                '''
                Accuracy statistics for GPS lock

                instance                  : Which instance of GPS we're reporting on (uint8_t)
                h_acc                     : GPS-reported horizontal accuracy (float)
                s_acc                     : GPS-reported speed accuracy (float)
                h_vel_filt                : GPS-reported, filtered horizontal velocity (float)
                v_vel_filt                : GPS-reported, filtered vertical velocity (float)
                p_drift                   : GPS position drift (float)
                ekf_check_mask            : Which fields pass EKF checks (uint8_t)

                '''
                return MAVLink_gps_accuracy_message(instance, h_acc, s_acc, h_vel_filt, v_vel_filt, p_drift, ekf_check_mask)

        def gps_accuracy_send(self, instance, h_acc, s_acc, h_vel_filt, v_vel_filt, p_drift, ekf_check_mask):
                '''
                Accuracy statistics for GPS lock
                ...
                '''
                return self.send(self.gps_accuracy_encode(instance, h_acc, s_acc, h_vel_filt, v_vel_filt, p_drift, ekf_check_mask))
